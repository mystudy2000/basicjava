    ...
    <p>
        <c:forEach var="contactEntry" items="${resume.contacts}">
            <jsp:useBean id="contactEntry"
                         type="java.util.Map.Entry<webapp.model.TypeOfContact, java.lang.String>"/>
    ...
    <table cellpadding="2">
        <c:forEach var="sectionEntry" items="${resume.sections}">
            <jsp:useBean id="sectionEntry"
                         type="java.util.Map.Entry<webapp.model.TypeOfSection, webapp.model.Section>"/>
            ...
            <c:choose>
                <c:when test="${type=='OBJECTIVE'}">
                    <tr>
                        <td colspan="2">
                            <h3><%=((StringSection) section).getStringSection()%></h3>
                        </td>
                    </tr>
                </c:when>
                <c:when test="${type=='PERSONAL'}">
                    <tr>
                        <td colspan="2">
                            <%=((StringSection) section).getStringSection()%>
                        </td>
                    </tr>
                </c:when>
                ...
                <c:when test="${type=='EXPERIENCE' || type=='EDUCATION'}">
                    <c:forEach var="org" items="<%=((OrganizationSection) section).getOrganizations()%>">
                        ...
                        <c:forEach var="position" items="${org.positions}">
                            <jsp:useBean id="position" type="webapp.model.Position"/>
                            <tr>
